...
export default async (): Promise<Response> => {
  try {
    ...
    const istruzioni = [
      ...
      "- `ρ_ij < 0.35 ∀ i≠j`",
      "- **Nessun limite di gambe per campionato**: non conta da quale campionato arriva la partita.",
      "- **Nessun limite di gambe per slot orario**: se le partite sono tutte alla stessa ora va bene lo stesso.",
      "- **Unico criterio di selezione: la qualità del pronostico.** Campionato e orario non escludono nulla. L'unico vincolo che resta sulle combinazioni è quello di correlazione (ρ < 0.35), che serve a non giocare due volte lo stesso evento.",
